...
def save_to_xlsx(df: pd.DataFrame,
    ...
    first_persona_col_letter = get_column_letter(2)  # now column B (LinkedIn URL is column A)
    ws.freeze_panes = f"{first_persona_col_letter}2"

    # 3‣ Data rows -----------------------------------------------------------
    for r, (_, row) in enumerate(dfv.iterrows(), start=2):
        # LinkedIn URL column (previously id value)
        id_val = row['id']
        linkedin_cell = ws.cell(row=r, column=1, value=id_val)
        if isinstance(id_val, str) and id_val.startswith(('http://', 'https://')):
            linkedin_cell.hyperlink, linkedin_cell.style = id_val, 'Hyperlink'

        c = 2  # persona columns start at column 2 now
